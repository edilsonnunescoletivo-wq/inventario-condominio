...
ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS sindico_id bigint REFERENCES usuarios(id) ON DELETE SET NULL;
...
ALTER TABLE condominios ADD COLUMN IF NOT EXISTS sindico_id bigint REFERENCES usuarios(id) ON DELETE RESTRICT;
...
-- Contas globais existentes permanecem Super Admin.
UPDATE usuarios SET tipo_conta='superadmin', licenca_status='ativo'
WHERE is_superadmin=true;

-- Administradores existentes são preservados. A vinculação como síndico pode ser feita pelo Super Admin.
UPDATE usuarios SET tipo_conta='sindico'
WHERE is_superadmin=false AND perfil='admin' AND tipo_conta='equipe';

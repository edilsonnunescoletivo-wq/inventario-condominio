...
export async function requireSession(request){
 ...
 const ur=await query(`SELECT id,nome,email,ativo,condominio_id,is_superadmin,senha_alterada_em,COALESCE(tipo_conta,CASE WHEN is_superadmin THEN 'superadmin' ELSE 'equipe' END) tipo_conta,sindico_id,licenca_status,licenca_inicio,licenca_fim,limite_condominios FROM usuarios WHERE id=$1`,[payload.sub]);
 ...
 if(base.tipo_conta==='equipe'&&base.sindico_id){const sr=await query(`SELECT id,is_superadmin,tipo_conta,licenca_status,licenca_inicio,licenca_fim FROM usuarios WHERE id=$1`,[base.sindico_id]);if(!licenseIsActive(sr.rows[0]))throw Object.assign(new Error('A licença do síndico responsável está inativa ou vencida.'),{status:403})}
 ...
 }else if(base.tipo_conta==='sindico'){
   const r=await query(`SELECT c.id condominio_id,'admin'::text perfil,c.nome condominio,c.cnpj,c.ativo condominio_ativo,c.status,c.modulos FROM condominios c WHERE c.id=$1 AND c.sindico_id=$2`,[wanted,base.id]);access=r.rows[0];
 }else{
